...
Locator: locator('button:has-text("Add to cart")')
Expected: "Add to cart"
Error: strict mode violation: locator('button:has-text("Add to cart")') resolved to 6 elements:
...
> 65 |   await expect(inventoryPage.addtocartButton).toHaveText("Add to cart");
     |                                               ^ Error: expect(locator).toHaveText(expected) failed
  66 | 
  67 |   //step 3 : Verify number on the cart
  68 |   await expect(inventoryPage.shoppingCartBadge).toHaveText("0");
  69 | });
  70 | 
```