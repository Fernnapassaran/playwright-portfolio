...
Call log:
  - waiting for locator('[data-test="inventory-item"]').filter({ hasText: 'Sauce Labs Backpack' }).locator(locator('button:has-text("Add to cart")')).getByRole('button', { name: /add to cart/i })
...
```ts
  ...
  38 | 
> 39 |     await addButton.getByRole("button", { name: /add to cart/i }).click();
     |                                                                   ^ Error: locator.click: Test timeout of 30000ms exceeded.
  40 |   }
  41 | 
